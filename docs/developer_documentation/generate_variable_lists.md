...
| reference_information/mappings.json | Variable data source file. | This is a required file called upon by the main script. This source file provides the stash and usage profile information for each variable, as well as any associated labels including the required "do-not-produce" status. |
| variables/<version_number>/*.txt | Output directory for the generated variable lists | A new directory is created for each new data request version leaving variable lists for previous versions stored safely in easily accessible directories. |